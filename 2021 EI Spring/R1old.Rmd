...
### Comparison of Athens and Phoenix after converting Athens to kWh
```{r}
conv <- 24/1000
s %>% 
  group_by(year(TimeStamp)) %>% 
  summarize(meanYear = mean(SolarWatt)*conv)
s2 <- s %>% 
  group_by(month = month(TimeStamp)) %>% 
  summarize(meanMonth = mean(SolarWatt)*conv)

url <- "http://www.richardtwatson.com/data/Phoenix.csv"
t <- read_delim(url, delim=',')

cor.test(s2$meanMonth,t$Solar)
```

# Conclusions
* The solar radiation data for Athens and Phoenix are correlated (p < .05). They have the same profile, but this does not mean the total radiation received is the same.
* Correlation is *large* (0.94)
